...
window.addEventListener("mousemove", (event) => {
  ...
  // Get all planet mesh objects
  const planetMeshes = planets.map((p) => p.planet);
  const intersects = raycaster.intersectObjects(planetMeshes);

  if (intersects.length > 0) {
    const planet = intersects[0].object;
    const index = planetMeshes.indexOf(planet);
...
const speedFolder = gui.addFolder("Planet Orbital Speeds");
planets.forEach((planet, index) => {
  const name = planetNames[index];
  speedFolder.add(planet, "rotaing_speed_around_sun", 0, 0.05).name(`${name} Orbit`);
});
speedFolder.open();
